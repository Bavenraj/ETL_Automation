...
table_list = [
    ['skyways_recon_data', 'hist_skyways_recon', 'REPORTING_DATE'], 
    ['trade_partner_recon', 'hist_trade_partner_data', 'Reporting_Date'],
    ['uae_inbound_recon', 'hist_uae_inbound', 'Reporting_Date'],
    ['luggage_coverage_data', 'hist_luggage_coverage', 'Reporting_Date'],
    ['falcon_air_2021', 'hist_falcon_air_2021', 'Reporting Date']
]
...
def data_check(**kwargs):
    ...
    for x, y, z in table_list:
        key = f"TRAVEL_1_Stage0_Task_State_{x.lower()}"
        task_id = f"TRAVEL_1_Monthly_Stage0_TaskGroup.TRAVEL_1_Stage0_Task_{x.lower()}"
        status = task_instance.xcom_pull(key=key, task_ids = task_id , dag_id = "TRAVEL_1_MONTHLY_STAGE_0", include_prior_dates = True)
...
with DAG(
    dag_id = "Etl_automation_1",
    default_args = default_args,
    schedule_interval = Variable.get("Hist_Table_Update_1"),
    catchup= False,
    tags = ['UPDATE_HIST_TRAVEL_1', 'MONTHLY_Update_TRAVEL_1']
) as dag:
